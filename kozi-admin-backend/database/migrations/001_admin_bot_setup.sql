-- Fixed Admin Bot Database Setup Migration
-- Remove the CREATE DATABASE and USE lines since database exists

-- User profiles table (admin users) - FIXED
...
-- Chat sessions table (admin chat sessions)
...
-- Chat messages table (admin conversations)
...
-- Payment schedules table (for payment reminders)
...
-- Employee data table (for database queries)
...
-- Employer data table (for database queries)
...
-- Email processing table (for Gmail AI support)
...
-- Platform analytics table (for admin reports)
...
-- Admin knowledge base table (for vector storage references)
CREATE TABLE IF NOT EXISTS admin_knowledge_base (
    knowledge_id INT AUTO_INCREMENT PRIMARY KEY,
    document_title VARCHAR(255) NOT NULL,
    document_type ENUM('admin_guide', 'payment_procedure', 'database_manual', 'email_template', 'policy') NOT NULL,
    content_summary TEXT,
    vector_path VARCHAR(500),
    tags JSON,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    is_active BOOLEAN DEFAULT TRUE,
...
-- Insert sample email processing data
INSERT IGNORE INTO email_processing (email_subject, sender_email, email_category, priority, content_summary, processing_status) VALUES
('Job Application Status Inquiry', 'john.doe@email.com', 'job_seeker_inquiry', 'medium', 'User asking about application status', 'pending'),
('Employer Verification Request', 'hr@newcompany.com', 'employer_request', 'high', 'Company requesting verification', 'pending'),
('Technical Support - Login Issues', 'user@kozi.rw', 'support_ticket', 'low', 'User having login trouble', 'draft_ready');